...
select sp.ProfileId from student_profiles as sp
join student_assignments sa
	on sp.ProfileId = sa.ProfileId
join student_classes sc
	on sa.AssignmentId=aa.AssignmentId
where sp.Active='on' and  aa.assignToId not in(select AssignedToId from assignment_assigned);
